 ...
 */
	public function do_multi_upload($field){
		...
		for($i=0; $i<count($_FILES[$field]["name"]); $i++){
			//continue if no file selected, skip file if empty
			if (isset($_FILES[$field]["skip"][$i]) && !$_FILES[$field]["name"][$i] && $i<count($_FILES[$field]["name"])-1) {
				continue;
			}
			...
			$this->upload_name	= isset($_FILES[$field]["upload_name"][$i]) ? $_FILES[$field]["upload_name"][$i] : "";
