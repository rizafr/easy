...
class Profile_Model extends MY_Model {
	...
	public function getProfile($id = NULL, $status = NULL)
	{
		$this->field = array(
							$this->user . '.' . '`user_id`',
							$this->user . '.' . '`role_id`',
		...
		$this->db->join($this->profile, $this->user . '.' . '`user_id`' . ' = ' . $this->profile . '.' . '`user_id`', '', FALSE);
		...
		if ($id) {
			$this->db->where($this->profile . '.' . '`profile_id`', $id);
		}
	...
	public function updateProfile($id,$data)
	{
		if ($id && $data) {
			...
			$query = $this->db->update($this->profile, $data);
...
/* Location: ./application/models/member/profile_model.php */